...
impl PySmartScheduler {
    ...
    pub fn generate_batch(&mut self, batch_size: usize) -> PyResult<SmartBatch> {
        ...
        while f_a_out.len() < batch_size {
            ...
            let ops = vec![
                Operand {
                    feature_idx: self.idx_a,
                    op_idx: self.op_a,
                },
                Operand {
                    feature_idx: self.idx_b,
                    op_idx: self.op_b,
                },
            ];
...
impl PySmartScheduler {
    ...
    // This order improves natural cache locality for Feature A and Feature B.
    // It does not reserve or pin hardware L2 cache.
    fn advance_state(&mut self) {
        // 1. Advance Interaction Type
        self.interact_type += 1;
        if self.interact_type < self.n_interact_types {
            return;
        }
        self.interact_type = 0;
