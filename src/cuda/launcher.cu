...
cudaError_t launch_decision_path_membership(
    ...
    dim3 block(threads);
    kernel::decision_path_membership_kernel<<<grid, block, 0, stream>>>(
        features,
        n_samples,
        n_features,
        terms,
...
void tune_cuda_kernels_for_device(const cudaDeviceProp& props) {
    ...
    static_cast<void>(cudaFuncSetCacheConfig(
        gafime_cuda_v1::kernel::decision_path_membership_kernel,
        shared_heavy_cache
    ));
...
int validate_decision_path_batch(const CudaMatrix* matrix, const GafimeDecisionPathBatch* batch) {
    if (matrix == nullptr || batch == nullptr || batch->abi_version != GAFIME_ABI_VERSION) {
        return GAFIME_STATUS_INVALID_ARGUMENT;
    }
    ...
    for (uint32_t path_idx = 0; path_idx < batch->path_count; ++path_idx) {
        ...
        for (uint32_t term_idx = begin; term_idx < end; ++term_idx) {
            ...
            if (term.feature >= matrix->cols || !decision_path_sign_supported(term.sign)) {
                return GAFIME_STATUS_INVALID_ARGUMENT;
            }
        }
    }
    ...
    if (matrix->rows != 0 && batch->path_count > UINT64_MAX / matrix->rows) {
        return GAFIME_STATUS_OUT_OF_MEMORY;
    }
    const uint64_t output_count = matrix->rows * static_cast<uint64_t>(batch->path_count);
...
GAFIME_GPU_API int gafime_gpu_decision_path_membership(
    ...
    auto* matrix = static_cast<CudaMatrix*>(matrix_handle);
    int status = validate_decision_path_batch(matrix, paths);
    if (status != GAFIME_STATUS_OK) {
        return status;
    }
    ...
    const uint64_t output_count = matrix->rows * static_cast<uint64_t>(paths->path_count);
