...
impl NanInfHandler {
    ...
    pub fn clean(
        data: &mut [Vec<f32>],
        nan_strategy: NanInfStrategy,
        inf_strategy: NanInfStrategy,
    ) {
    ...
    fn calculate_mean(data: &[f32]) -> f32 {
        let valid: Vec<f32> = data.iter().filter(|x| x.is_finite()).copied().collect();
        if valid.is_empty() {
            return 0.0;
        }
    ...
    fn calculate_median(data: &[f32]) -> f32 {
        let mut valid: Vec<f32> = data.iter().filter(|x| x.is_finite()).copied().collect();
        if valid.is_empty() {
            return 0.0;
        }
...
impl DataOrienter {
    ...
    pub fn analyze_missing(data: &[Vec<f32>]) -> MissingValueReport {
        ...
        let missing_by_feature: Vec<(usize, usize, f32)> = data
            .iter()
            .enumerate()
            .map(|(idx, feature)| {
        ...
        let missing_by_row: Vec<(usize, usize)> = row_missing
            .iter()
            .enumerate()
        ...
        let features_with_missing: Vec<usize> = missing_by_feature
            .iter()
            .filter(|(_, c, _)| *c > 0)
            .map(|(idx, _, _)| *idx)
            .collect();
        let rows_with_missing: Vec<usize> = missing_by_row.iter().map(|(idx, _)| *idx).collect();
    ...
    pub fn check_alignment(data: &[Vec<f32>]) -> Result<usize, Vec<(usize, usize)>> {
        ...
        let misaligned: Vec<(usize, usize)> = data
            .iter()
            .enumerate()
...
impl EntropyAnalyzer {
    ...
    pub fn analyze_feature(&self, feature: &[f32], feature_idx: usize) -> FeatureEntropyReport {
        ...
        let (feature_type, recommendation) =
            self.classify_feature(normalized_entropy, unique_count, unique_ratio, n);
    ...
    fn classify_feature(
        ...
        if unique_count <= self.categorical_max_unique {
            return (
                ...
                format!(
                    "CATEGORICAL: {} unique values - consider encoding",
                    unique_count
                ),
            );
        }
    ...
    pub fn get_features_to_drop(&self, reports: &[FeatureEntropyReport]) -> Vec<usize> {
        reports
            ...
            .filter(|r| {
                matches!(
                    r.feature_type,
                    FeatureType::Constant | FeatureType::LowVariance
                )
            })
            .map(|r| r.feature_idx)
            .collect()
    ...
    pub fn get_id_features(&self, reports: &[FeatureEntropyReport]) -> Vec<usize> {
        reports
            ...
            .filter(|r| {
                matches!(
                    r.feature_type,
                    FeatureType::UniqueId | FeatureType::HighCardinality
                )
            })
            .map(|r| r.feature_idx)
            .collect()
...
impl PyDataQualityAnalyzer {
    ...
    fn analyze_missing(
        &self,
        data: Vec<Vec<f32>>,
    ) -> (bool, usize, usize, usize, Vec<usize>, Vec<usize>) {
    ...
    fn analyze_entropy(
        &self,
        data: Vec<Vec<f32>>,
    ) -> Vec<(usize, f32, usize, f32, String, String)> {
