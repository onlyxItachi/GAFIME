...
type PipelineSubmitFn =
    unsafe extern "C" fn(*mut c_void, *const i32, *const i32, *const i32, i32, *mut i32) -> i32;
...
fn get_cuda_lib() -> Result<&'static Library, String> {
    ...
    // Slow path: load library from an explicit override or common local/package
    // build locations. Avoid user-machine hardcoded paths.
    let mut paths = Vec::new();
    if let Some(path) = env::var_os("GAFIME_CUDA_LIB") {
        paths.push(PathBuf::from(path));
    }
    paths.extend([
        PathBuf::from("gafime_cuda.dll"),
        PathBuf::from("libgafime_cuda.so"),
        PathBuf::from("gafime/gafime_cuda.dll"),
        PathBuf::from("gafime/libgafime_cuda.so"),
    ]);
...
impl AsyncPipelineProducer {
    ...
    pub fn new(bucket_ptr: usize, val_fold_id: i32) -> Result<Self, String> {
        ...
        let result = unsafe { fn_init(bucket_ptr as *mut c_void, val_fold_id, &mut pipeline) };
    ...
    pub fn wait(&mut self) -> Result<BatchResult, i32> {
        ...
        let result = unsafe { (self.fn_wait)(self.pipeline, stats.as_mut_ptr(), &mut batch_size) };
    ...
    pub fn poll(&mut self) -> Option<BatchResult> {
        ...
        let result = unsafe { (self.fn_poll)(self.pipeline, stats.as_mut_ptr(), &mut batch_size) };
    ...
    pub fn process_all(
        &mut self,
        all_batches: Vec<(Vec<i32>, Vec<i32>, Vec<i32>)>,
    ) -> Vec<BatchResult> {
...
impl PyAsyncPipeline {
    ...
    fn submit(&mut self, indices: Vec<i32>, ops: Vec<i32>, interact: Vec<i32>) -> PyResult<i32> {
        ...
        producer.submit(&indices, &ops, &interact).map_err(|e| {
            PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("Submit error: {}", e))
        })
    }
    ...
    fn wait(&mut self) -> PyResult<(Vec<f32>, usize)> {
        ...
        let result = producer.wait().map_err(|e| {
            PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("Wait error: {}", e))
        })?;
    ...
    fn process_all(
        &mut self,
        batches: Vec<(Vec<i32>, Vec<i32>, Vec<i32>)>,
    ) -> PyResult<Vec<(Vec<f32>, usize)>> {
        let producer = self
            .inner
            .as_mut()
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>("Pipeline closed"))?;
