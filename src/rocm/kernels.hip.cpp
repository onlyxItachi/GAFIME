...
static void auto_tune_for_gpu_impl(int device_id) {
    ...
    // =========================================================================
    // AUTO-TUNE BLOCK SIZE based on HIP architecture
    // =========================================================================
    int compute_cap = g_gpu_config.compute_major * 10 + g_gpu_config.compute_minor;
    
    if (compute_cap >= 120) {
        // Future AMD GPU generations: keep high queue pressure.
        g_gpu_config.block_size = 256;
        g_gpu_config.max_blocks = props.multiProcessorCount * 8;
    } else if (compute_cap >= 110) {
        // RDNA3/RDNA4-class HIP targets, including gfx11xx APUs/dGPUs.
        g_gpu_config.block_size = 256;
        g_gpu_config.max_blocks = props.multiProcessorCount * 8;
    } else if (compute_cap >= 90) {
        // CDNA/RDNA2 era HIP targets.
        g_gpu_config.block_size = 256;
        g_gpu_config.max_blocks = props.multiProcessorCount * 4;
    } else {
        // Conservative fallback for older/unknown HIP targets.
        fprintf(stderr, "[GAFIME] WARNING: unknown ROCm/HIP architecture %d.%d; using conservative tuning.\n",
...
GAFIME_API int gafime_rocm_bucket_free(GafimeRocmBucket bucket) {
    ...
    if (impl->d_target) hipFree(impl->d_target);
    if (impl->d_mask) hipFree(impl->d_mask);
...
__device__ __forceinline__ float discrete_sigmoid(float z) {
    ...
    return 1.0f / (1.0f + expf(-z_clamped));
}
...
GAFIME_API int gafime_discrete_soft_batch_rocm(
    ...
    err = hipMalloc(&d_X, X_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_y, y_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_kinds, int_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_feature_a, int_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_feature_b, int_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_value_feature, int_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_directions, int_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_params, params_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_scales, scales_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_sharpness, sharpness_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_stats, stats_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }

    err = hipMemcpy(d_X, h_X, X_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_y, h_y, y_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_kinds, h_kinds, int_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_feature_a, h_feature_a, int_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_feature_b, h_feature_b, int_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_value_feature, h_value_feature, int_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_directions, h_directions, int_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_params, h_params, params_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_scales, h_scales, scales_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_sharpness, h_sharpness, sharpness_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemset(d_stats, 0, stats_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    ...
    err = hipGetLastError();
    if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipDeviceSynchronize();
    if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(h_stats_batch, d_stats, stats_bytes, hipMemcpyDeviceToHost);
    if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }

cleanup:
    if (d_stats) hipFree(d_stats);
    if (d_sharpness) hipFree(d_sharpness);
    if (d_scales) hipFree(d_scales);
    if (d_params) hipFree(d_params);
    if (d_directions) hipFree(d_directions);
    if (d_value_feature) hipFree(d_value_feature);
    if (d_feature_b) hipFree(d_feature_b);
    if (d_feature_a) hipFree(d_feature_a);
    if (d_kinds) hipFree(d_kinds);
    if (d_y) hipFree(d_y);
    if (d_X) hipFree(d_X);
    return status;
}
...
GAFIME_API int gafime_discrete_selection_batch_rocm(
    const float* h_X,
    const float* h_y,
    ...
    err = hipMalloc(&d_X, X_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_y, vec_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_residual, vec_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_kinds, int_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_feature_a, int_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_feature_b, int_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_value_feature, int_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    ...
    err = hipMalloc(&d_scales, scales_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_sharpness, sharpness_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_scores, scores_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }

    err = hipMemcpy(d_X, h_X, X_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_y, h_y, vec_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_residual, h_residual, vec_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_kinds, h_kinds, int_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_feature_a, h_feature_a, int_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_feature_b, h_feature_b, int_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_value_feature, h_value_feature, int_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    ...
    err = hipMemcpy(d_scales, h_scales, scales_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_sharpness, h_sharpness, sharpness_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemset(d_scores, 0, scores_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    ...
    err = hipDeviceSynchronize();
    if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(h_scores_batch, d_scores, scores_bytes, hipMemcpyDeviceToHost);
    if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
...
extern "C" GAFIME_API int gafime_discrete_selection_adaptive_rocm(
    const float* h_X,
    const float* h_y,
    const float* h_residual,
    const int* h_y_bins,
    ...
    err = hipMalloc(&d_y, vec_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_residual, vec_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_y_bins, sample_int_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_kinds, int_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_feature_a, int_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_feature_b, int_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    err = hipMalloc(&d_value_feature, int_bytes); if (err != hipSuccess) { status = GAFIME_ERROR_OUT_OF_MEMORY; goto cleanup; }
    ...
    err = hipMemcpy(d_y, h_y, vec_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_residual, h_residual, vec_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_y_bins, h_y_bins, sample_int_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_kinds, h_kinds, int_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_feature_a, h_feature_a, int_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_feature_b, h_feature_b, int_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
    err = hipMemcpy(d_value_feature, h_value_feature, int_bytes, hipMemcpyHostToDevice); if (err != hipSuccess) { status = GAFIME_ERROR_KERNEL_FAILED; goto cleanup; }
