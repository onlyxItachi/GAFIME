...
| **P-D** legacy surface | 🟡 mostly done — `GafimeSelector` sklearn transformer ported + tested; **export via `CompileFlags(export=True)`** wired (zero-copy Arrow capsule pair on the compiled artifact) + tested; saturating wide-count planning already present (`combos.rs`); telemetry present (`tools/telemetry.py`). Resident-session reuse + whole-sweep graph still open. |
...
| **P-E** ROCm parity | 🟡 **continuous pearson/r2 + MI + significance validated on gfx1150** — ROCm lib builds (`gpu/rocm`, gfx1150), ABI-generic backend wired into gpu-sys (`rocm_from_env`) + Python (`backend="rocm"`/`"hip"` → `v1-rocm-cabi`); **MI kernel ported from CUDA (matches CUDA MI within 1e-3 on hardware)**; gpu-sys parity + Python e2e pass on real AMD hardware. ROCm top-k / graph still open. |
| **P-G** Metal | ⏳ honest stub — no Apple hardware; `backend="metal"` reports a clear capability error; Metal-4 design (residency sets / command buffers / MTLTensor) documented in §0/P-G. Real path deferred until Apple HW. |
| **P-B / P-H / P-I** | ⏳ open (GPU spearman = per-candidate device sort; ROCm MI = HIP kernel port; CPU MI-SIMD = perf; RT-core OptiX; autotune). |
