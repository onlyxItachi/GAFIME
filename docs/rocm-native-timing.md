...
loads the exact shared object named by `--payload` and resolves only the
canonical ABI 1.1 symbols (`gafime_gpu_*_v2`). It does not link the product
library, call private kernels, or change the public ABI.

The helper runs fp32, mixed, and fp64 routes in all six deterministic profile
orders. Each order exercises ingest conversion, candidate descriptor
materialization, host planning, allocation, upload, Pearson/R2/MI/Spearman,
the route-typed Spearman target-rank build after target replacement, the
subsequent cached unary Spearman route, top-k plus selected-row gather, result
...
ranks and reduces/returns double; fp64 uses double throughout. Metric/ranking samples use
synchronized HIP events (`hipDeviceSynchronize`, event start/stop, and
`hipEventSynchronize`); host-only boundaries use `steady_clock`. The JSON
artifact records raw samples, route/workload configuration, source commit,
payload/helper hashes, HIP runtime/driver/device details, environment, CPU
affinity, and clock metadata.
