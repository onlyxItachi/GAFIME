...
class NativeRocmBackend(Backend):
    ...
    def _setup_graph_functions(self) -> None:
        ...
        if hasattr(self.lib, "gafime_rocm_matrix_compute_ts_batch"):
            self.lib.gafime_rocm_matrix_compute_ts_batch.restype = ctypes.c_int
            self.lib.gafime_rocm_matrix_compute_ts_batch.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_int),
                ctypes.POINTER(ctypes.c_int),
                ctypes.POINTER(ctypes.c_int),
    ...
    def score_time_series_candidates_resident(
        ...
        """HIP mirror of resident TS scoring (Option B). Reads the resident matrix
        by global feature index. Read path is mode-agnostic; the session gates the
        in-place y swap on ``supports_resident_target_update`` (UMA -> fallback)."""
        if not hasattr(self.lib, "gafime_rocm_matrix_compute_ts_batch"):
            raise RuntimeError(
                "Resident time-series scoring requires gafime_rocm_matrix_compute_ts_batch; "
                "rebuild the ROCm payload."
            )
        ...
        for batch in _chunks_objects(candidates_list, GAFIME_MAX_BATCH_SIZE):
            ...
            rc = self.lib.gafime_rocm_matrix_compute_ts_batch(
                matrix,
                _int_array(kinds),
                _int_array(feats),
            ...
            if rc != GAFIME_SUCCESS:
                raise RuntimeError(f"gafime_rocm_matrix_compute_ts_batch failed with code {rc}")
