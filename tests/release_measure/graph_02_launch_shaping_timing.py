...
def main():
    ...
    if plain <= 0 or graph <= 0 or not math.isfinite(speedup):
        raise AssertionError(
            f"invalid graph timing: plain={plain} graph={graph} speedup={speedup}"
        )
    print(
        f"[{backend}] plain={plain / 1e6:.2f}ms  graph={graph / 1e6:.2f}ms  speedup={speedup:.3f}x"
    )
    rec = tel.new_record(
    ...
    rec["results"].update(
        {
            "status": "pass",
            "plain_launch_ns": int(plain),
            "graph_replay_ns": int(graph),
            "graph_speedup": round(speedup, 4),
        }
    )
    tel.write_run(rec, mc.OUTDIR)
    print(
        f"logged real speedup (may be ~1.0 — no claim beyond the artifact). {mc.OUTDIR}"
    )
