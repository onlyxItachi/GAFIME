...
fn main() {
    ...
    let candidate_parallelism = match variant.as_str() {
        "baseline" => "frozen_pre_repair_serial_candidate_loop",
        "candidate" => "rayon_candidate_level",
        _ => panic!("GAFIME_PRODUCTION_VARIANT must be baseline or candidate"),
    };
    let semantic_candidate_participation_guard = match variant.as_str() {
        "baseline" => "not_applicable_frozen_pre_repair_serial_baseline",
        "candidate" => "cfg_test_precision_executor_parallelism_contract",
        _ => unreachable!("variant was checked above"),
    };
    ...
    let report = format!(
        ...
        json_string(candidate_parallelism),
        ...
        json_string(semantic_candidate_participation_guard),
